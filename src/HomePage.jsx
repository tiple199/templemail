      ...
      <Layout style={{ minHeight: '100vh', background: darkMode ? '#141414' : '#f0f2f5' }}>
        <Header style={{ background: darkMode ? '#1f1f1f' : '#001529', display: 'flex', justifyContent: 'center', alignItems: 'center', padding: '10px 20px', height: 'auto' }}>
          <Space size="large">
            ...
            {!isPremium ? <Button type="primary" danger ghost size="small" onClick={() => setPremiumModal(true)}>Nâng cấp</Button> : <Popconfirm title="Thoát Premium?" onConfirm={() => setIsPremium(false)}><Button type="default" danger size="small" icon={<LogoutOutlined />}>Rời Premium</Button></Popconfirm>}
        ...
        <Content style={{ padding: '20px', maxWidth: '1200px', margin: '0 auto', width: '100%' }}>
          {isPremium && savedAddresses.length > 0 && (
            <Card size="small" style={{ ...cardStyle, marginBottom: 16 }} title={<Text style={titleColor}><PushpinOutlined /> Đã ghim</Text>}>
              <Space wrap>{savedAddresses.map(h => <Badge key={h} count={<DeleteOutlined style={{ color: 'red' }} onClick={() => setSavedAddresses(savedAddresses.filter(a => a !== h))} />} offset={[-5, 5]}><Button type={email === h ? "primary" : "default"} size="small" onClick={() => { setEmail(h); setEmails([]); setSelectedEmail(null); }}>{h}</Button></Badge>)}</Space>
            </Card>
          )}
          ...
          <Row gutter={16}>
            <Col xs={24} md={8}>
              <Card title={<Text style={titleColor}>Danh sách thư ({emails.length})</Text>} style={cardStyle}>
                <List dataSource={emails} locale={{ emptyText: <Empty description="Chưa có thư" /> }}
                  renderItem={(item) => (
                    <List.Item onClick={() => setSelectedEmail(item)} style={{ cursor: 'pointer', padding: '12px', borderRadius: '8px', marginBottom: '8px', background: selectedEmail?.id === item.id ? (darkMode ? '#303030' : '#e6f7ff') : 'transparent' }}>
                      <List.Item.Meta avatar={<Badge dot={selectedEmail?.id !== item.id}><MailOutlined style={{ color: '#1890ff' }} /></Badge>} title={<Text strong style={titleColor} ellipsis>{item.subject}</Text>} description={<Text type="secondary" style={{ fontSize: '11px' }}>{item.sender.split('<')[0]}</Text>} />
                    </List.Item>
                  )}
                />
              </Card>
            ...
            <Col xs={24} md={16}>
              <Card title={<Text style={titleColor}>Chi tiết nội dung</Text>} style={{ ...cardStyle, minHeight: '400px' }} extra={selectedEmail && isPremium && <Button type="link" onClick={() => { setArchivedEmails([{...selectedEmail, archiveId: Date.now()}, ...archivedEmails]); message.success("Đã lưu!"); }}>Lưu thư</Button>}>
                {selectedEmail ? (
                  <div>
                    <Title level={4} style={titleColor}>{selectedEmail.subject}</Title>
                    <Divider />
                    <div style={{ padding: '16px', background: '#fff', borderRadius: '8px', color: '#333', overflowX: 'auto', minHeight: '300px' }} dangerouslySetInnerHTML={{ __html: DOMPurify.sanitize(selectedEmail.content) }} />
                  </div>
                ) : <Empty description="Chọn một thư để xem" style={{ marginTop: '100px' }} />}
              </Card>
            </Col>
          </Row>
          ...
          {isPremium && archivedEmails.length > 0 && (
            ...
            <List dataSource={archivedEmails} renderItem={(item) => <Card size="small" style={{ marginBottom: 8 }} extra={<Space><Button size="small" icon={<EyeOutlined />} onClick={() => setViewingArchive(item)}>Xem</Button><Button size="small" danger onClick={() => setArchivedEmails(archivedEmails.filter(a => a.archiveId !== item.archiveId))} icon={<DeleteOutlined />} /></Space>}><Text>{item.subject}</Text></Card>} /></>
          )}
        </Content>

        <Modal title="Nâng cấp Premium" open={premiumModal} onOk={() => { if(premiumCode === "PREMIUM2024") { setIsPremium(true); setPremiumModal(false); message.success("Kích hoạt!"); } else message.error("Sai mã!"); }} onCancel={() => setPremiumModal(false)}><Input placeholder="PREMIUM2024" value={premiumCode} onChange={e => setPremiumCode(e.target.value)} /></Modal>
